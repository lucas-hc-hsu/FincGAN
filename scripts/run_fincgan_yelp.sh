#!/bin/bash
###############################################################################
# FincGAN Automated Training Script for Yelp Dataset
#
# This script automates the complete FincGAN training pipeline for Yelp:
...
#
# Usage: ./run_fincgan_yelp.sh [OPTIONS]
###############################################################################
...
UU_THRESHOLD=0.91
UP_THRESHOLD=0.99
GRAPH_DIR="./graph_output/yelp/"
RESULT_DIR="./results/yelp/"
EMBED_DIR="./embed/yelp/"
GENERATOR_DIR="./generator/yelp/"
TSNE_DIR="./tsne/yelp/"
TMP_DIR="./tmp/yelp/"
FIGURES_DIR="./figures/yelp/"
VERBOSE=1
SKIP_STAGES=""
...
FincGAN Automated Training Pipeline for Yelp Dataset
...
check_dataset() {
    print_info "Checking Yelp dataset..."
    if [ ! -f "$DATA_DIR" ]; then
        print_error "Yelp dataset not found at: $DATA_DIR"
        exit 1
    fi
    print_success "Yelp dataset found at: $DATA_DIR"
}
...
create_directories() {
    ...
    run_cmd mkdir -p "$EMBED_DIR"
    run_cmd mkdir -p "$GENERATOR_DIR"
    run_cmd mkdir -p "$TSNE_DIR"
    run_cmd mkdir -p "$TMP_DIR"
    run_cmd mkdir -p "$FIGURES_DIR"
    print_success "Directories created"
}

# Display configuration
display_config() {
    print_header "FincGAN Training Configuration (Yelp Dataset)"
    cat << EOF
  Dataset             : Yelp
  Data Path           : $DATA_DIR
  GPU ID              : $GPU_ID
  ...
  Output Directories:
    ...
    - Generators      : $GENERATOR_DIR
    - t-SNE Plots     : $TSNE_DIR
    - Figures         : $FIGURES_DIR
...
stage_embedding() {
    ...
    if [ ! -f "${EMBED_DIR}yelp_hgt_user_emb.pt" ] || [ ! -f "${EMBED_DIR}yelp_hgt_product_emb.pt" ]; then
        print_error "Embeddings not found after Stage I"
        exit 1
    fi

    print_success "Stage I completed - Embeddings saved to $EMBED_DIR"
}
...
stage_node_generator() {
    ...
    if [ ! -f "${GENERATOR_DIR}yelp_D.pt" ] || [ ! -f "${GENERATOR_DIR}yelp_G.pt" ]; then
        print_error "GAN models not found after Stage II"
        exit 1
    fi

    print_success "Stage II completed - GAN models saved to $GENERATOR_DIR"

    if [ -f "${TSNE_DIR}tsne.jpg" ]; then
        print_info "t-SNE visualizations saved to $TSNE_DIR"
    fi
}
...
stage_edge_generators() {
    ...
    run_cmd python3 -m fincgan.edge_generator_uu \
        ...
        --data_dir "$DATA_DIR" \
        --emb_dir "$EMBED_DIR" \
        --edge_dir "$GENERATOR_DIR" \
        --edge_generator_verbose "$VERBOSE" \
        --dataset-name "$DATASET"

    if [ ! -f "${GENERATOR_DIR}uu_generator.pt" ]; then
    ...
    if [ ! -f "${GENERATOR_DIR}up_generator.pt" ]; then
        print_error "UP generator not found after training"
        exit 1
    fi
    print_success "User-Product edge generator trained"

    print_success "Stage III completed - Edge generators saved to $GENERATOR_DIR"
}
...
stage_visualization() {
    ...
    # Move the generated figure to yelp-specific directory
    if [ -f "figures/figure_3.png" ]; then
        run_cmd mv "figures/figure_3.png" "${FIGURES_DIR}figure_3_yelp.png"
        print_success "Stage V completed - Visualization saved to ${FIGURES_DIR}figure_3_yelp.png"
    else
...
display_results() {
    print_header "Training Results Summary (Yelp Dataset)"

    if [ -f "$RESULT_DIR/yelp_hgt_model_gan.txt" ]; then
        print_info "FincGAN Results:"
        echo ""
        python3 << EOF
import pandas as pd
try:
    df = pd.read_csv("$RESULT_DIR/yelp_hgt_model_gan.txt")
    if len(df) > 0:
        print("  Dataset: Yelp")
...
EOF
        ...
        print_warning "Results file not found: $RESULT_DIR/yelp_hgt_model_gan.txt"
    fi

    print_info "Generated Files:"
    echo ""
    echo "  Embeddings:"
    [ -f "${EMBED_DIR}yelp_hgt_user_emb.pt" ] && echo "    ✓ ${EMBED_DIR}yelp_hgt_user_emb.pt"
    [ -f "${EMBED_DIR}yelp_hgt_product_emb.pt" ] && echo "    ✓ ${EMBED_DIR}yelp_hgt_product_emb.pt"
    echo ""
    echo "  GAN Models:"
    [ -f "${GENERATOR_DIR}yelp_D.pt" ] && echo "    ✓ ${GENERATOR_DIR}yelp_D.pt"
    [ -f "${GENERATOR_DIR}yelp_G.pt" ] && echo "    ✓ ${GENERATOR_DIR}yelp_G.pt"
    echo ""
    echo "  Edge Generators:"
    [ -f "${GENERATOR_DIR}uu_generator.pt" ] && echo "    ✓ ${GENERATOR_DIR}uu_generator.pt"
    [ -f "${GENERATOR_DIR}up_generator.pt" ] && echo "    ✓ ${GENERATOR_DIR}up_generator.pt"
    echo ""
    echo "  Visualizations:"
    [ -f "${FIGURES_DIR}figure_3_yelp.png" ] && echo "    ✓ ${FIGURES_DIR}figure_3_yelp.png"
    [ -d "$TSNE_DIR" ] && [ "$(ls -A $TSNE_DIR)" ] && echo "    ✓ ${TSNE_DIR}*.jpg ($(ls ${TSNE_DIR}*.jpg 2>/dev/null | wc -l) files)"
    echo ""
}
...
main() {
    ...
    print_header "FincGAN Training Pipeline for Yelp Dataset"
    ...
    echo "  - View visualizations in: $FIGURES_DIR"
    echo "  - View t-SNE plots in: $TSNE_DIR"
    echo ""
}
