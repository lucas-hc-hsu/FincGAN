...
UU_THRESHOLD=0.91
UP_THRESHOLD=0.99
GRAPH_DIR="./graph_output/amazon/"
RESULT_DIR="./results/amazon/"
EMBED_DIR="./embed/amazon/"
GENERATOR_DIR="./generator/amazon/"
TSNE_DIR="./tsne/amazon/"
TMP_DIR="./tmp/amazon/"
FIGURES_DIR="./figures/amazon/"
VERBOSE=1
SKIP_STAGES=""
...
stage_embedding() {
    ...
    if [ ! -f "${EMBED_DIR}${DATASET}_hgt_user_emb.pt" ] || [ ! -f "${EMBED_DIR}${DATASET}_hgt_product_emb.pt" ]; then
...
stage_node_generator() {
    ...
    if [ ! -f "${GENERATOR_DIR}${DATASET}_D.pt" ] || [ ! -f "${GENERATOR_DIR}${DATASET}_G.pt" ]; then
...
stage_visualization() {
    ...
    run_cmd python3 -c "from fincgan.visualize import auto_plot_figure_3; auto_plot_figure_3(result_dir='$RESULT_DIR', save_fig=True, save_dir='$FIGURES_DIR')"

    if [ -f "${FIGURES_DIR}figure_3.png" ]; then
        print_success "Stage V completed - Visualization saved to ${FIGURES_DIR}figure_3.png"
    else
