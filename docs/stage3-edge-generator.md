...
**Solution**: Reduce batch size (requires code modification) or use CPU:
```bash
python3 edge_generator_uu.py --gpu_id -1
```

### Issue 3: Model File Not Saved

**Solution**: Ensure `generator/` directory exists:
```bash
mkdir -p generator
```

### Issue 4: Poor Edge Quality in Next Stage

**Solution**:
- Increase training epochs for UU generator (try 150-200)
- Adjust thresholds in Stage IV
- Verify embeddings quality from Stage I

## Next Steps

After successfully training both edge generators:

1. **Verify both model files exist**
2. **Proceed to Stage IV**: [Graph Generation and Training](stage4-training.md)
