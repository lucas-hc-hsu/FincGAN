...
### Issue 1: Generator/Discriminator Loss Imbalance

**Symptom**: One loss goes to 0 while the other explodes

**Solution**: Adjust learning rate or training iterations:
```bash
python3 node_generator.py --lr 0.0001 --n_epochs 10
```

### Issue 2: Mode Collapse

**Symptom**: t-SNE shows all fake nodes in one cluster

**Solution**:
- Increase training epochs
- Adjust architecture (requires code modification)
- Try different random seeds

### Issue 3: t-SNE Visualization Fails

**Symptom**: Error during t-SNE generation

**Solution**: Install/update scikit-learn:
```bash
pip install --upgrade scikit-learn
```

### Issue 4: Out of Memory During t-SNE

**Symptom**: CUDA/CPU OOM during visualization

**Solution**: Reduce t-SNE sample size (requires code modification) or use CPU:
```bash
...
1. **Review t-SNE plots**: Verify generation quality
2. **Proceed to Stage III**: [Edge Generator](stage3-edge-generator.md)
